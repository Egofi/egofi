...
import type { KybOverview, KybDocumentDto } from "@egofi/types";
...
const TIER2_DOCS = [KybDocumentType.ProofOfAddress, KybDocumentType.BankStatement, KybDocumentType.TaxId];
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-md text-sm text-navy-500">
                {status === KybStatus.Pending && "Upload the documents below and submit to raise your limits."}
                {status === KybStatus.UnderReview && "We're reviewing your documents — usually 1–2 business days."}
                {verified && `You're verified at ${currentTier?.label ?? `Tier ${overview.tier}`}.`}
                {status === KybStatus.Rejected && (overview.reviewNote || "Some documents need attention. Update them and resubmit.")}
              </p>
            </div>
            <div className="text-right">
              <p className="text-[11px] font-medium uppercase tracking-wide text-navy-400">Current limit</p>
                ...
                <div key={tier.tier} className="flex flex-1 items-center gap-2">
                  <div className="flex flex-1 flex-col gap-1.5">
                    <div
                      className={cn(
                        "h-1.5 rounded-full",
                        reached ? "bg-primary" : "bg-navy-100",
                      )}
                    />
                    <div className="flex items-center gap-1.5">
                      <span className={cn("text-xs font-medium", isCurrent ? "text-primary" : reached ? "text-navy-700" : "text-navy-400")}>
                        Tier {tier.tier}
                      </span>
                      {isCurrent && <span className="size-1.5 rounded-full bg-primary" />}
                    </div>
                    <span className="text-[11px] text-navy-400">{formatCap(tier.volumeCapUsd)}</span>
                  </div>
                  {i < overview.tiers.length - 1 && <span className="hidden text-navy-200 sm:block">›</span>}
                </div>
              );
            })}
          </div>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="space-y-3">
              {TIER1_DOCS.map((t) => (
                <DocumentSlot key={t} type={t} document={docByType(t)} required locked={locked} onUpload={handleUpload} onDelete={handleDelete} />
              ))}
            </div>
            ...
            <div className="space-y-3">
              {TIER2_DOCS.map((t) => (
                <DocumentSlot key={t} type={t} document={docByType(t)} required={false} locked={locked} onUpload={handleUpload} onDelete={handleDelete} />
              ))}
            </div>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-navy-100 pt-6">
              <p className="text-sm text-navy-500">
                {locked ? "Submitted — you'll be notified once review is complete."
                  : hasTier1 ? "We'll review your documents and set your tier."
